...
class SemanticConstructor[N <: Arity](using line: sourcecode.Line, file: sourcecode.File)(
  ...
  def semanticSignature(vars: Seq[Variable[Ind]]): Seq[(Variable[Ind], Expr[Ind])] = vars
    .zip(underlying.specification.map(_.getOrElse(adt.specializedTerm(typeVariablesSeq))))
  ...
  val typ: Expr[Ind] =
    semanticSignature.unzip._2.foldRight[Expr[Ind]](adt.specializedTerm(typeVariablesSeq))((a, b) => (a ->: b))
  ...
  val classFunction: Constant[HeadSort] =
    DEF(using name = fullName)(classFunctionExpr.asInstanceOf[Expr[HeadSort]])
  ...
  private def buildInductiveCase(selfCarrier: Expr[Ind]): Expr[Prop] = syntacticSignature
    .foldRight[Expr[Prop]](P(appliedTerm1)) { (el, fc) =>
      val (v, typ) = el
      typ match
        case SelfRef => forall(v, in(v, selfCarrier) ==> (P(v) ==> fc))
        // case RegularArg(t) => forall(v, (v :: typeExprToTerm(t)) ==> fc)
        case TypeArg(name) => forall(v, in(v, typeExprToTerm(name)) ==> fc)
    }

  /** Case generated by this constructor when performing a proof by induction. */
  lazy val inductiveCase: Expr[Prop] = buildInductiveCase(adt.term)

  /**
   *  Variant of [[inductiveCase]] using the specialized ADT carrier for self-recursive
   *  arguments.
   */
  lazy val specializedInductiveCase: Expr[Prop] =
    buildInductiveCase(adt.specializedTerm(typeVariablesSeq))
}
